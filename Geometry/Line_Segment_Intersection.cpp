...
struct Pt {
    ...
    Pt operator/(T k) { return { x / k, y / k }; }
    T operator*(Pt<T> p) { return x * p.x + y * p.y; }
    T operator^(Pt<T> p) { return x * p.y - y * p.x; }
...
void solve() {
    ...
    auto [intersectionType, _, __] = segmentIntersection(l1, l2);
