...
void solve() {
    ...
    // The absolute value is taken because the area could be negative
    // depending on the order of the points (clockwise or counterclockwise)
    area = abs(area);
    ...
    auto countBoundaryPoints = [](const vector<Point<i64>>& polygon) -> i64 {
        ...
        for (int i = 0; i < n; i++) {
            ...
            result += gcd(abs(dx), abs(dy));
        }
        return result;
    };
    
    i64 A = area;
    i64 b = countBoundaryPoints(polygon);
    i64 i = (A - b + 2) / 2;
