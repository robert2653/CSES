...
//     multiset<Point<i64>, sortY> st;
//     for (int i = 0, l = 0; i < n; i++) {
//         while (l < i && a[i].x - a[l].x >= ans) st.erase(st.find(a[l++]));
//         for (auto it = st.lower_bound(Point<i64>(a[i].x, a[i].y - ans)); it != st.end() && it->y - a[i].y < ans; it++) {
//             if (distanceSquare(*it, a[i]) < ans) {
//                 ans = distanceSquare(*it, a[i]);
//             }
//         }
//         st.insert(a[i]);
//     }
