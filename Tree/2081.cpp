    ...
    auto dfs = [&](auto self, int u, int pre) -> void {
        ...
        for (auto v : tree[u]) {
            ...
            for (auto [i, j] : vmp[v]) {
                ...
                // }
                auto up = vmp[u].upper_bound(k2 - !swapped - i - lazy[u] - lazy[v]);
                auto down = vmp[u].lower_bound(k1 - !swapped - i - lazy[u] - lazy[v] - 1);
                if (up == vmp[u].end()) --up;
                if (down == vmp[u].end()) --down;
                ans += (up->second - down->second) * j;
            }
            for (auto [i, j] : vmp[v]) {
